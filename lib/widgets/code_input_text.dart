...
class UnderlineInputCell extends StatelessWidget {
  ...
  const UnderlineInputCell(
      {Key? key, this.isFocused:true, this.text, this.obscureText: false})
      : super(key: key);
...
class CircleInputCell extends StatelessWidget {
  ...
  CircleInputCell({Key? key, this.isFocused:true, this.text, this.obscureText: false})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          border: Border.all(
              color:
                  isFocused ? Colors.red : Colors.grey,
              width: 1.5)),
      child: obscureText
          ? text == '' ? SizedBox() : Icon(Icons.brightness_1, size: 20)
          : Text(text ?? '', style: TextStyle(fontSize: 22)),
